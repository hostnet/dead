...
class AstFilesVisitor extends AbstractNodeElementVisitor
{
    ...
    {
        $xml
                ->registerXpathNamespace("ast",
                        "http://www.phpcompiler.org/phc-1.0");
    }

    private function parse_ast(Ast $ast)
    {
        $xml = new SimpleXMLElement($ast->getAst(), NULL, false,
                "http://www.phpcompiler.org/phc-1.0");
        ...
        foreach ($classes as $class) {
            $this->parse_class($class);
        }
    }

    private function parse_class(SimpleXMLElement $class)
    {
    ...
    private function parse_method(SimpleXMLElement $method)
    {
        ...
        foreach ($invocations as $invocation) {
            $this->parse_invocation($invocation);
        }
        echo "\n";
    }

    private function parse_invocation(SimpleXMLElement $invocation)
    {
        $dynamic = false;
